...
if (isset($_POST['saveGuru'])) {
    ...
    for($i = 2; $i <= $jumlah_baris; $i++){
        $bidangilmu = $data->val($i, 1);
	   $matakuliah = $data->val($i, 2);
	   $kode = $data->val($i, 3);
	    $prasyarat = $data->val($i, 4);

        if($matakuliah != "" && $sks != "" && $kode != "" && $prasyarat != ""){
            //persiapkan insert data ke database
        $save = mysqli_query($con, "INSERT INTO tb_it_pilihan VALUES ('', '$bidangilmu', '$matakuliah', '$kode',  '$prasyarat') ");
            $berhasil++;
        }
    }
    unlink($_FILES['file']['name']);
        if ($save) {
            echo "
				...
				window.location.replace('?page=it_pilihan');
				} ,3000);   
				</script>";
        }
